...
# Замени эти ссылки на свой реальный домен
MINI_APP_URL = "https://074903fab006ae.lhr.life" 
RULES_URL = "loquacious-donut-e20cc1.netlify.app" 
...
def add_order(order: OrderCreate):
    ...
    cur = conn.execute("""
        INSERT INTO orders (title, description, text, author, author_id, type, time, price)
        VALUES (?, ?, ?, ?, ?, ?, NULL, ?)
...
def respond_to_order(order_id: int, data: RespondRequest):
    ...
    order = conn.execute("SELECT id, title, author, author_id, price FROM orders WHERE id = ?",(order_id,)).fetchone()
    if not order:
        raise HTTPException(status_code=404, detail="Объявление не найдено.")
...
def get_my_orders(telegram_id: int):
    cleanup_old_orders()
    rows = conn.execute("""
        SELECT o.id, o.title, o.description, o.type, o.price, 
